/**
 * Gets a random avatar URL from the public/avatars folder (server-side)
 * @returns Promise<string | null> - The public URL of a random avatar, or null if none found
 */
export async function getRandomAvatarServer(): Promise<string | null> {
  try {
    ...
    console.log(`Selected random avatar (server): ${randomAvatar}`)
